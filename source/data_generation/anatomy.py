...
def get_phase(config, t):
    ...
    assert t >= - \
        config.THETA_MAX and t <= config.THETA_MAX, 't = {} is out of range'.format(
            t)

    t = config.GANTRY2HEART_SCALE*t

    assume_repetition =True
    if assume_repetition:
        t = t % 1

    assert t >= 0 and t <= 1, 'Resultant t = {} is out of range (0,1)'.format(
        t)
...
class Motion:
    def __init__(self, config):
        ...
        self.config = config
        # Radius size:
        # Motion primitives used throughout the experiments:
        # const        : radius stays fixed across the timeline
        # simple_sin   : single sinusoid scaling the radius
        # coupled_sin  : combined sin/cos pair to get richer size beats
        # pseudo_heart : hand-crafted piecewise curve that mimics a heartbeat
        # figure_eight : parametric infinity-shaped translation of the centre

        # Location paths:
        # rectangle    : superellipse-style translation that traces a rectangle
        # const2       : zero translation (keeps centre fixed)
        # simple_sin2  : 1-D sinusoid translation along x
        # circle       : circular trajectory for the centre
    ...
    def get_motion(self, size, location):
        assert isinstance(size, str) and isinstance(
            location, str), 'size and location are string parameters'
        assert size in list(self.func_map.keys()) and location in list(self.func_map.keys(
        )), 'Wrong key words for size:{} and location:{}'.format(size, location)
    ...
    def const(self, val, t):
        ...
        assert isinstance(
            t, float) and t >= 0 and t <= 1, 't: {} must be a float between 0 and 1'.format(t)
        return val
    ...
    def simple_sin(self, val, t, A=.4, bias=1.0):
        ...
        if result > 3*val:
            warnings.warn(
                'Resultant value of motion func is more than 3 times the val!!!')
    ...
    def pseudo_heart(self, val, t):
        ...
        A = np.array([[T[2], T[2]**2, T[2]**3],
                      # Matrix for polynomial function computation
                      [T[3], T[3]**2, T[3]**3],
                      [0, 2, 3*(T[2]+T[3])]])
...
class Organ:
    '''
    Parametric moving organ modeled as an ellipse. The center (cx, cy) and radii
    (a, b) are modulated over time via the motion functions chosen in
    __init__. To support other shapes, replace/extend the ellipse() check
    used by is_inside().
    '''
    ...
    def __init__(self, config, center, a, b, size, location):
        ...
        assert isinstance(
            config, Config), 'config must be an instance of class Config'
        assert isinstance(center, list) and len(center) == 2 and all([isinstance(
            c, float) for c in center]), 'center must be a list of 2 float coordinates'
        assert min(center) >= 0 and max(
            center) <= 1, 'center coordinates must be between 0 and 1'
        assert isinstance(a, float) and isinstance(
            b, float) and a > 0 and b > 0, 'radii a and b should be positive float'
        assert isinstance(size, str) and isinstance(
            location, str), 'size and location are string parameters'
    ...
    def ellipse(self, pt, a, b):
        assert isinstance(pt, np.ndarray) and len(
            pt.shape) == 2, 'pt must be a 2D numpy array'
        assert not np.isnan(pt).any(), 'NaN value present in array pt'
        assert isinstance(a, float) and isinstance(
            b, float) and a > 0 and b > 0, 'radii a and b should be positive float'

        dist = ((pt[:, 0]-self.center[0])/a)**2 + \
            ((pt[:, 1]-self.center[1])/b)**2 - 1
        result = ((dist < 0)*1).reshape(-1, 1)

        assert (((result == 0) + (result == 1)) ==
                True).all(), 'Not all values are either 0 or 1'
        assert len(result.shape) == 2, 'result has incorrect shape of {}'.format(
            result.shape)
    ...
    def is_inside(self, pt, t):
        ...
        assert isinstance(pt, np.ndarray) and len(
            pt.shape) == 2, 'pt must be a 2D numpy array'
        assert isinstance(t, float) and abs(
            t) <= self.config.THETA_MAX, 'Time is out of range: {}'.format(t)
...
class Body:
    ...
    def __init__(self, config, organs: List[Organ]):

        assert isinstance(
            config, Config), 'config must be an instance of class Config'
        assert isinstance(organs, list), 'Provide a valid list of organs'
        assert all([isinstance(organ, Organ) for organ in organs]
                   ), 'List elements must be of type organ'
        assert len(
            organs) == config.INTENSITIES.shape[1], 'Number of organs should match number of intensities'
    ...
    def is_inside(self, pt, t):
        ...
        assert isinstance(pt, np.ndarray) and len(
            pt.shape) == 2, 'Points must be 2D numpy array'
        assert isinstance(t, float) and abs(
            t) <= self.config.THETA_MAX, 'Time is out of range: {}'.format(t)
        ...
        assert (np.sum(inside, axis=1) < 2).all(
        ), 'Organs are intersecting !! '
        return inside
